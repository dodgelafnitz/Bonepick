  ...
  */
...
#else
        SetValue(j, i, AsciiImageData(' ', 0,
          byte(imageColors.GetValue(j, i))));
#endif
      }
    }
  }
  ...
  void AsciiImage::Render(void) const
  {
    ...
    for (unsigned i = 0; i < GetCount(); ++i)
    {
      ...
      info[i].Attributes = GetAttributesFromColors(
        imageData.foregroundColorIndex,
  ...
  void GetColorPalette(Color * colors, unsigned colorCount, unsigned offset)
  {
    ...
    for (unsigned i = 0; i < colorCount; ++i)
    {
      colors[i] = GetColorFromApplyColor(
        screenBufferInfo.ColorTable[offset + i]);
    }
  }
}
